...
function validateLatitude(lat) {
    if (isNaN(lat) || lat < -90 || lat > 90) {
        throw new ValidationError(`Invalid latitude: ${lat}. Latitude must be between -90 and 90 degrees.`);
    }
}
/**
...
function validateLongitude(lon) {
    if (isNaN(lon) || lon < -180 || lon > 180) {
        throw new ValidationError(`Invalid longitude: ${lon}. Longitude must be between -180 and 180 degrees.`);
    }
}
// ============================================================================
 ...
 */
function findNearestSA2(lat, lon) {
...
async function handleGetSupplyPipeline(postcode) {
    ...
    if (approvalsData.error && stockData.error) {
        return createErrorResponse(`Unable to retrieve supply pipeline data for postcode ${postcode}.\n` +
 ...
 */
async function handleGetLocationStats(lat, lon) {
    validateLatitude(lat);
    validateLongitude(lon);
    // Check cache status
    if (!cacheInitialized) {
        return createErrorResponse(`Geography cache not yet initialized. Cannot lookup location.\n` +
            `Error: ${cacheError || 'Cache initialization in progress'}`);
    }
    // Find nearest SA2 region
    const nearest = findNearestSA2(lat, lon);
    if (!nearest) {
        return createErrorResponse(`Unable to find nearest SA2 region for coordinates (${lat}, ${lon}).\n` +
    ...
    const data = await fetchABSData(endpoint);
    if (data.error) {
        return createErrorResponse(`Unable to retrieve location statistics for coordinates (${lat}, ${lon}).\n` +
    ...
    try {
        ...
        switch (name) {
            case 'get_location_stats':
                return await handleGetLocationStats(args.latitude, args.longitude);
            ...
            default:
                return createErrorResponse(`Unknown tool: "${name}". Available tools are: get_location_stats, get_suburb_stats, get_mortgage_stress, ` +
